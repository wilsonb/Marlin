...
#define HOMEAXIS_DO(LETTER) \
  ...
  if (axis==X_AXIS ? HOMEAXIS_DO(X) :
    ...
    // Engage Servo endstop if enabled
      ...
      #if defined (ENABLE_AUTO_BED_LEVELING) && (PROBE_SERVO_DEACTIVATION_DELAY > 0)
        if (axis==Z_AXIS) {
         // engage_z_probe(); Line removed to use fixed end stop for z instead of servo end stop during homing.  Fixed stop must be in parallel (NO switch) with probe stop.
        }
	    else
      #endif
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 points.
        {
            ...
            bool zig = true;
            do_blocking_move_to(current_position[X_AXIS], current_position[Y_AXIS], Z_RAISE_BEFORE_PROBING); //raise head to allow room to engage probe
            ...
            for (int yProbe=FRONT_PROBE_BED_POSITION; yProbe <= BACK_PROBE_BED_POSITION; yProbe += yGridSpacing)
            {
              ...
              for (int xCount=0; xCount < ACCURATE_BED_LEVELING_POINTS; xCount++)
              {
                ...
                //retract_z_probe();  Do not lift probe between measurements -BW edit
            ...
            retract_z_probe(); //Complete probe retract after level
